...
from utils import load_content_style_image, load_vgg_CNN, initialize_generated_image, generate_selected_model, tensor_to_image, \
...
def run_simulation(path_image_content, path_image_style, path_image_output):
    ...
    # TODO preprocess (why?)
    content_output = vgg_model_selected(tf.Variable(tf.image.convert_image_dtype(content_image, tf.float32)))
    style_output = vgg_model_selected(tf.Variable(tf.image.convert_image_dtype(style_image, tf.float32)))
